...
| GitHub Actions, second runs (2026-09-22) | `bench-android` 3.13.0 → 3.14.0 passed end to end with `ITERATIONS=3`: 112 rows, one fingerprint, sentinel noise floor +2.66 %; timing verdicts are `inconclusive` at n=3 by design (see docs/CONVENTIONS.md §10). `bench-apple` 2.1.0 → 2.2.0 produced size, init (3 rounds) and 2.1.0 in-process rows, then failed in the 2.2.0 in-process pass: the first `send` reached the stub and was answered, but the SDK completion did not fire for about 200 s and the 30 sends issued behind it queued (2.1.0 passed on the same runner; not reproducible locally). Harness changes in response: run-loop waits (`XCTWaiter`) instead of a main-thread semaphore, a stub that closes gracefully, times out idle connections and accepts chunked / `100-continue` uploads, metrics recorded only for clean passes, and a failed pass is retried once and reported in the comment instead of aborting the job. Rerun pending |
